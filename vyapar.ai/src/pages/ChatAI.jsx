...
import VyaparLogo from '../components/ui/VyaparLogo';
import { Sparkles, User, Send, Mic, MicOff, Volume2 } from 'lucide-react';
    ...
    <div className="flex flex-col h-[calc(100vh-180px)] animate-slideUp relative overflow-hidden rounded-3xl border border-indigo-500/30 shadow-2xl">
      ...
      <div className="relative z-10 bg-gray-900/50 p-4 border-b border-gray-700 flex items-center justify-between backdrop-blur-md">
        <div className="flex items-center gap-3">
          <div className="w-10 h-10 rounded-xl bg-gray-800/50 flex items-center justify-center border border-white/10 shadow-lg">
            <VyaparLogo size={28} />
          </div>
          <div>
      ...
      <div className="relative z-10 flex-1 overflow-y-auto p-6 space-y-6">
        {messages.map(msg => (
          <div key={msg.id} className={`flex items-end gap-3 ${msg.sender==='user' ? 'justify-end' : 'justify-start'}`}>
            {msg.sender==='ai' && (
              <div className="w-8 h-8 rounded-full bg-gray-800 flex-shrink-0 flex items-center justify-center border border-indigo-500/30">
                <VyaparLogo size={18} />
              </div>
            )}
            ...
            <div className={`max-w-[80%] p-4 rounded-2xl shadow-lg transition-all duration-300 ${msg.sender==='user' ? 'bg-gradient-to-r from-indigo-600 to-blue-600 text-white rounded-br-none' : 'bg-gray-800 border border-gray-700 text-gray-100 rounded-bl-none'}`}>
              ...
              <div className="flex items-center justify-between mt-2">
                <span className={`text-[10px] opacity-60 ${msg.sender==='user' ? 'text-right' : 'text-left'}`}>{new Date().toLocaleTimeString([], {hour:'2-digit', minute:'2-digit'})}</span>
                {msg.sender === 'ai' && (
                  <button 
                    onClick={() => speakText(msg.text)}
                    className="ml-2 p-1.5 rounded-full hover:bg-gray-700/50 transition-colors"
                    title="Play audio"
                  >
                    <Volume2 size={14} className={`${isSpeaking ? 'text-green-400 animate-pulse' : 'text-gray-400'}`} />
                  </button>
                )}
              </div>
            </div>
            ...
            {msg.sender==='user' && (
              <div className="w-8 h-8 rounded-full bg-gray-700 border border-gray-600 flex-shrink-0 flex items-center justify-center">
      ...
      <div className="relative z-10 p-4 bg-gray-900/50 backdrop-blur-md border-t border-gray-800">
        <form onSubmit={handleSend} className="flex gap-3 max-w-4xl mx-auto">
          <button
            ...
            className={`${isRecording ? 'bg-red-600 hover:bg-red-500 animate-pulse' : 'bg-gray-700 hover:bg-gray-600'} text-white p-3 rounded-xl transition-all shadow-lg active:scale-95 flex items-center justify-center w-12`}
