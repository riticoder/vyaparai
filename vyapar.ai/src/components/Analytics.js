        ...
        <TiltCard className="bg-gray-800 p-6 rounded-2xl border border-gray-700">
          ...
          <div className="space-y-4">
            {[
              ...
              <div key={i}>
                <div className="flex justify-between text-sm text-gray-400 mb-1"><span>{item.name}</span><span>{item.val}%</span></div>
                <div className="w-full bg-gray-700 rounded-full h-2 overflow-hidden">
                  <div className={`h-full ${item.col} relative`} style={{width: `${item.val}%`}}>
